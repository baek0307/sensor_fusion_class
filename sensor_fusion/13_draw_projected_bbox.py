...
def get_2d_bboxes(camera_box_points):
    ...
    for box_pts in camera_box_points:
        # 2D BBOX를 정의하기 위해 u, v 좌표만 사용합니다.
        u_coords = box_pts[:, 0]
        v_coords = box_pts[:, 1]

        # 2D BBOX의 좌상단과 우하단 포인트를 정의합니다.
        u_min, v_min = u_coords.min(), v_coords.min()
        u_max, v_max = u_coords.max(), v_coords.max()

        # 좌상단과 우하단 좌표를 리스트로 저장합니다.
...
def draw_2d_boxes(image, camera_2d_box_points):
    ''' 2D BBOX를 이미지에 그리기 '''
    for bbox in camera_2d_box_points:
        top_left = tuple(map(int, bbox[0]))
        bottom_right = tuple(map(int, bbox[1]))

        # 두께 3, 색상 녹색의 2D BBOX를 이미지에 그립니다.
...
def main_pipeline(left_image, lidar_bin, velo_points):
    ...
    left_image = draw_2d_boxes(left_image, camera_2d_box_points)
