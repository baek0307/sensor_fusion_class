...
# 빈 검정 화면 생성 (left_image와 동일한 크기)
black_canvas = np.zeros_like(left_image)

# 포인트를 검정 화면에 표시
for i in range(len(u)):
    color = cv2.applyColorMap(np.array([[z_normalized[i]]], dtype=np.uint8), cv2.COLORMAP_RAINBOW)[0][0]
    cv2.circle(black_canvas, (int(u[i]), int(v[i])), 1, color.tolist(), -1)

# 결과 보여주기
cv2.imshow('Camera Detection', cv2.cvtColor(annotated_image, cv2.COLOR_RGB2BGR))
cv2.imshow('Lidar Projected Points', black_canvas)
cv2.waitKey(0)
